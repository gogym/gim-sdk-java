...
public class LiveKitSfuAdapter implements SfuAdapter {
    ...
    /**
     * Server API 地址（去除尾部斜杠）
     */
    private String serverHost() {
        String host = config.getHost();
        return host != null ? host.replaceAll("/$", "") : "http://127.0.0.1:7880";
    }

    /**
     * 下发给客户端的 SFU 连接地址（未配置时由 serverHost 推导）
     */
    private String wsUrl() {
        String wsUrl = config.getWsUrl();
        return wsUrl != null && !wsUrl.isEmpty() ? wsUrl : serverHost().replace("http", "ws");
    }

    /**
     * 接入 token 有效期（秒，未配置时默认 3600）
     */
    private int tokenTtlSeconds() {
        return config.getTokenTtlSeconds() > 0 ? config.getTokenTtlSeconds() : 3600;
    }
    ...
    private void callServerApi(String path, String jsonBody) {
        try {
            ...
            String serviceToken = buildJwt(config.getApiKey(), video);

            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create(serverHost() + path))
    ...
    private String buildJwt(String identity, JsonObject videoGrants) {
        ...
        payload.addProperty("iss", config.getApiKey());
